...
function bill(id: string, createdAt: string, amount: number, price: number, consumption: number, type = "BOLLETTA_LUCE", profileId?: string) {
  return {
    id,
    profileId,
    type,
...
function payroll(id: string, createdAt: string, net: number, gross: number, contributions: number, irpef: number, profileId?: string) {
  return {
    id,
    profileId,
    type: "BUSTA_PAGA",
...
test("non confronta categorie di bolletta diverse", () => {
  assert.deepEqual(buildLongitudinalInsights([
    bill("luce", "2026-08-30T10:00:00Z", 120, 0.22, 280, "BOLLETTA_LUCE"),
    bill("gas", "2026-07-30T10:00:00Z", 100, 0.18, 250, "BOLLETTA_GAS"),
  ]), []);
});

test("non confronta documenti di profili diversi anche se tipo e date sono compatibili", () => {
  assert.deepEqual(buildLongitudinalInsights([
    bill("me", "2026-08-30T10:00:00Z", 140, 0.24, 300, "BOLLETTA_LUCE", "profile-me"),
    bill("brother", "2026-07-30T10:00:00Z", 90, 0.17, 210, "BOLLETTA_LUCE", "profile-brother"),
  ]), []);
});

test("confronta solo dentro lo stesso profilo quando ci sono più soggetti", () => {
  const insights = buildLongitudinalInsights([
    payroll("me-aug", "2026-08-30T10:00:00Z", 1500, 2200, 250, 320, "profile-me"),
    payroll("brother-aug", "2026-08-29T10:00:00Z", 2200, 3100, 330, 500, "profile-brother"),
    payroll("me-jul", "2026-07-30T10:00:00Z", 1650, 2200, 230, 270, "profile-me"),
  ]);
  assert.equal(insights.length, 1);
  assert.equal(insights[0].currentDocumentId, "me-aug");
  assert.equal(insights[0].previousDocumentId, "me-jul");
});

test("ignora documenti non completati", () => {
  const pending = bill("pending", "2026-08-30T10:00:00Z", 150, 0.25, 300);
  pending.status = "PENDING";
  assert.deepEqual(buildLongitudinalInsights([pending, bill("done", "2026-07-30T10:00:00Z", 100, 0.20, 250)]), []);
});
