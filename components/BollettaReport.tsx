                ...
                <div className="flex justify-between items-baseline mt-1 pl-4">
                  <span className="text-xs text-[#64748B]">
                    ...
                    {data.consumi?.valore && data.materia_energia?.quota_variabile_prezzo_kwh
                      ? ` (${data.consumi.valore} ${data.consumi.unita} × ${fmtKwh(data.materia_energia.quota_variabile_prezzo_kwh)} €/kWh)`
                      : ""}
                  </span>
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 mb-4">
            ...
            {confronto.kwh_mese_stimati !== null && (
              <Stat label="kWh/mese stimati" value={`${confronto.kwh_mese_stimati}`} mono />
            )}
          </div>

          {/* Indicatore */}
          {confronto.percentuale_sopra_media !== null && (
            <div className={`rounded-lg px-4 py-3 mb-4 ${confronto.percentuale_sopra_media > 5 ? "bg-red-50" : "bg-green-50"}`}>
              <p className={`text-sm font-medium ${confronto.percentuale_sopra_media > 5 ? "text-red-700" : "text-green-700"}`}>
                {confronto.percentuale_sopra_media > 5
                  ? `Stai pagando circa il ${Math.abs(confronto.percentuale_sopra_media)}% in più della media di mercato sulla materia energia`
              ...
              return (
                <div className="rounded-lg bg-blue-50 border border-blue-100 px-4 py-3 mb-4">
                  <p className="text-sm text-blue-800">
                    💡 <strong>{minKwh.provider}</strong> ha il prezzo al kWh più basso ({fmtKwh(minKwh.prezzo_kwh)} €/kWh)
                    {minKwh.quota_fissa_mensile ? ` ma con quota fissa di ${fmt(minKwh.quota_fissa_mensile, 0)}/mese` : ""}.
                    {confronto.kwh_mese_stimati && (
                      <> Con i tuoi {confronto.kwh_mese_stimati} kWh mensili, <strong>{migliore.provider}</strong> ti conviene di più.
                      {minKwh.break_even_kwh !== null && (
                        <> {minKwh.provider} diventerebbe conveniente oltre {minKwh.break_even_kwh} kWh/mese.</>
                      )}</>
                    )}
                  </p>
                </div>
            ...
            <>
                      ...
                      <tr key={i} className="border-b border-[#E2E8F0] last:border-0 hover:bg-[#F7F9FC]">
                        ...
                        <td className="py-3 pr-3 text-right font-mono">
                          {offerta.risparmio_mensile !== null
                            ? <span className={offerta.risparmio_mensile > 0 ? "text-[#10B981] font-semibold" : "text-[#64748B]"}>
                                {offerta.risparmio_mensile > 0 ? "-" : "+"}{fmt(Math.abs(offerta.risparmio_mensile))}/mese
                              </span>
                            : <span className="text-[#94A3B8]">—</span>
                          }
                        </td>
              ...
              {confronto.arera_prezzo_kwh && (
                <p className="text-xs text-[#94A3B8] mt-3">
                  Riferimento ARERA (tutela vulnerabili): {fmtKwh(confronto.arera_prezzo_kwh)} €/kWh
                </p>
              )}
            </>
          )}
